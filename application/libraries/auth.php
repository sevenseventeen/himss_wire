...
// 
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		...
		//If it is the incorrect email address;
		if(!$user) {
		...
		if ( $this->ci->ion_auth_model->forgotten_password($email) )   //changed
		{
            ...
            $from_email = $this->ci->config->item('email_from_support');
            $from_name = $this->ci->config->item('email_name_from_administrator');
//			$this->ci->email->from($this->ci->config->item('admin_email', 'ion_auth'), $this->ci->config->item('site_title', 'ion_auth'));
	...
	public function get_user_details($email_address) {
		$this->db->select('*');
